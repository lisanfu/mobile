...
public class MessageActivity extends TabActivity implements TabHost.OnTabChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState, PersistableBundle persistentState) {
        ...
        mTabHost.addTab(mTabHost.newTabSpec("tab1").setIndicator("拦截记录",getResources().getDrawable(R.drawable.tab_icon_1))
        .setContent(R.id.tabhost_public_listview));
        mTabHost.addTab(mTabHost.newTabSpec("tab2").setIndicator("黑名单",getResources().getDrawable(R.drawable.tab_icon_2))
        ...
        mDbAdapter.open();
        initTab1Data();
        initTabViewListener(mListViewTab1);
    }
    ...
    private void actionSMS(int position ,Cursor c,String column)
    {
        ...
        mIntent.setType("vnd.adnroid-dir/mms-sms");
        startActivity(mIntent);
    }
    /** 删除标记*/
    private void delMarkItemData(ListView lv,Cursor c,String table)
    {
        int mContent =lv.getCount();
        for(int i=0;i!=mContent;i++)
        {
    ...
    {
        for(int i=0;i!=mCursorTab1.getCount();i++)
        {
                ...
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        if (mCallName.getText().toString().length() != 0 && mCallNumber.getText().toString().length() != 0)
                        {
                            ...
                            builder.withValue(ContactsContract.CommonDataKinds.StructuredName.DISPLAY_NAME, mCallName.getText().toString());

                            operationList.add(builder.build());
                            builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
                            builder.withValueBackReference(ContactsContract.CommonDataKinds.Phone.RAW_CONTACT_ID, 0);
                            builder.withValue(ContactsContract.Data.MIMETYPE, ContactsContract.CommonDataKinds.Phone.CONTENT_ITEM_TYPE);
                            builder.withValue(ContactsContract.CommonDataKinds.Phone.NUMBER, mCallNumber.getText().toString());
    ...
    /** 多选是打电话只拨打第一个被选中的号码，发短信给第一个被选中的号码发送*/
        ...
        if(lv.getCheckedItemPositions().size()>=1)
        {
            for(int i=0;i!=lv.getCount();i++)
            {
                if(lv.getCheckedItemPositions().get(i)) {
                    ...
                    } else if (sms_call.equals(addContact)) {
                        if ("addContact".equals("call")) {
                            addContact(phone);
                        } else {
    ...
    /**--------------------------点击不同标签时的事件监听-------------------------------------------*/
    private boolean isCheckedTabOrTab2=false;
    ...
    public void onTabChanged(String tabId) {
        ...
        {
            isCheckedTabOrTab2=false;
            initTab1Data();
        }
        else if("tab2".equals(tabId))
        {
            isCheckedTabOrTab2=true;
            initTab2Data();
        }
    }
    private int i=0;
    /** ----------------------------从数据库中获取并加载短信垃圾数据Start-----------------------------------*/
    ...
    /** 从数据库中获得并加载垃圾短信数据end*/
    /** 列表事件的长按事件监听Start*/
    private void initTabViewListener(final ListView listView)
    {
        listView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            @Override
            public boolean onItemLongClick(AdapterView<?> adapterView, View view, int i, long l) {
                Toast.makeText(MessageActivity.this,mArray[i],Toast.LENGTH_LONG).show();
                return false;
            }
        });
    }
                     ...
                     String mNewForbidPhone=mForbidPhone.getText().toString();
                             ...
                             }else
                             {
                                 ...
                                 for(int i=0;i!=mStr.length;i++)
                                 {
                                     ...
                                     {
                                         Message_Rubbish_Info mMFI=new Message_Rubbish_Info();
                                         mMFI.setMessage_rubbish_phone(phone);
    ...
    {
        Cursor mCursor=getContentResolver().query(Contacts.Phones.CONTENT_URI,new String[]{"_id","name","number"},null,null,"number desc");
        startManagingCursor(mCursor);
        if(mCursor.moveToFirst())
        {
            int m=0;
            String mContent[]=new String[mCursor.getCount()];
            do
            {
                String phoneNumber=mCursor.getString(mCursor.getColumnIndex(Contacts.Phones.NUMBER));
                mContent[m]=mCursor.getString(mCursor.getColumnIndex(Contacts.Phones.NAME))+":"+(phoneNumber.length()==13?phoneNumber.substring(2):phoneNumber);
                m++;

            }while(mCursor.moveToNext());
            initListData(mContent);
        }
    }
    String checkInfo;
