package com.example.hp.mobile.db;
...
public class DbAdapter {
    ...
    //5个表，短信黑名单表，短信垃圾箱表，来电黑名单表，拉电缆街记录表
    public static final String MESSAGE_FORBID_TABLE_NAME="mesage_forbid_table";
    ...
    //短信黑名单字段，来电号码
    public final static String MESSAGE_FORBID_PHONE="message_forid_phone";
    //短信垃圾表字段，垃圾短信号码，收件时间，短信内容
    public static final String MESSAGE_RUBBISH_PHONE="message_rubbish_phone";
    public static final String MESSAGE_RUBBISH_TIME="message_fubbish_time";
    ...
    public Cursor getPhone(String phone,int i)
    {
        ...
        {
            case 1:
                mReturn=mSQLiteDatabase.rawQuery("select * from "+MESSAGE_RUBBISH_TABLE_NAME+" where "+MESSAGE_RUBBISH_PHONE
                        +"="
                        +"\'"+phone+"\'",null);
                break;
            case 2:
                mReturn=mSQLiteDatabase.rawQuery("select * from "+MESSAGE_FORBID_TABLE_NAME+" where "+MESSAGE_FORBID_PHONE
                        +"="
                        +"\'"
                        +phone+"\'",null);
                break;
            case 3:
                mReturn=mSQLiteDatabase.rawQuery("select * from "+CALL_RECORD_TABLE_NAME+" where "+CALL_RECORD_PHONE
                        +"="
                        +"\'"
                        +phone+"\'",null);
                break;
            case 4:
                mReturn=mSQLiteDatabase.rawQuery("select * from "+CALL_FORBID_TABLE_NAME+" where "+CALL_FORBID_PHONE
                        +"="
                        +"\'"
                        +phone+"\'",
                        null);
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper
    {
        ...
        public void initCreateSQL()
        {
            ...
            mStringBuilder_1.append(" text ");
            mStringBuilder_1.append(" );");
            Log.d(TAG,"MESSG_RUBBISH_TABLE_NAME");
        ...
        public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
            sqLiteDatabase.execSQL("drop table if exists"+MESSAGE_RUBBISH_TABLE_NAME,null);
            Log.d(TAG,"onUpgrade");
        }
    }
